...
function displayCards(){
    $flag=false;
    $names = array("Redouan", "Chloe", "Keven", "Berenice"); //array of usernames
    $users = array("Redouan", "Chloe", "Keven", "Berenice");  //key value pairs of names and scores
...
function getCards($user){
    ...
    while($total < 35){
        ...
        if($card_numbers[($card * $suite) - 1] != 0){
            ...
            $total += $card;
            echo "<img id='cards' src='cards/$suite/$card.png' alt='$card' title='' />"; //display card
        }
